package com.avatye.adcash.platform.provider.adpopcorn.bannerad
...
import com.avatye.adcash.platform.provider.adpopcorn.bannerad.loader.APsspBannerLoader
import com.avatye.adcash.platform.provider.adpopcorn.bannerad.loader.APsspBannerLoaderBase
import com.avatye.adcash.platform.provider.adpopcorn.bannerad.loader.APsspBannerLoaderCallback
import com.avatye.adcash.platform.provider.adpopcorn.bannerad.loader.APsspBannerNativeLoader
...
class APsspBanner(
...
) : AdsviserBanner(), APsspBannerLoaderCallback {

    private val sourceName = "APsspBanner"
    private val loaderQueues: Queue<AdsviserBannerProperty> = LinkedList()
    private val weakContext = WeakReference(context)
    private var currentLoader: APsspBannerLoaderBase? = null
    ...
    private fun fillLoaderQueue(bannerProperty: AdsviserBannerProperty) {
        weakContext.isAvailable {
            if (bannerProperty.unitSize == AdsviserBannerUnitSize.DYNAMIC) {
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
                    loaderQueues.add(bannerProperty)
                } else {
                    printout.info(sourceName = sourceName) {
                        "init::fillLoaderQueue API(${android.os.Build.VERSION.SDK_INT}) Pass -> # $bannerProperty # "
                    }
                }
            } else {
                loaderQueues.add(bannerProperty)
    ...
    override fun onLoaded(view: View, networkUnitName: String) {
        ...
        weakContext.isAvailable {
            currentLoader?.produce {
                printout.info(sourceName = sourceName) {
                    "onLoaded { loader: ${it.loaderName}, network: $networkUnitName }"
                }
                callback.onLoaded(
                    adView = view,
    ...
    private fun loaderFactory(property: AdsviserBannerProperty): APsspBannerLoaderBase? {
        if (weakContext.get() != null) {
            return when (property.unitType) {
                AdsviserBannerUnit.BANNER -> APsspBannerLoader(
                    context = context,
                    placementAppKey = placementAppKey,
                    placementId = property.placementId,
                    placementSize = property.unitSize,
                    mediationExtra = property.mediationExtra,
                    storeUrls = storeUrls,
                    callback = this@APsspBanner
                )

                AdsviserBannerUnit.NATIVE -> APsspBannerNativeLoader(
                    context = context,
                    placementAppKey = placementAppKey,
                    placementId = property.placementId,
                    placementSize = property.unitSize,
