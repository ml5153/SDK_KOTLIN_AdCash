...
enum class AMsspErrorUnit(val code: Int, val message: String) {
    EXCEPTION(
    code = 9000,
    message = "exception"
    ),
    ...
    NOT_EXISTS_AD(
    code = 9200,
    message = "not exists advertises[no-ad] { ${AdsviserProviderUnit.ADMIXER.providerName}: 5002 }"
    ),
    NOT_EXISTS_QUEUE(
    code = 9200,
    ...
    code = 9200,
    message = "invalid native spot dose not initialize { ${AdsviserProviderUnit.ADMIXER.providerName}: 5001 }"
    ),
    FAIL_OPEN(
    code = 9300,
    ...
    internal companion object {
        ...
        fun of(errorUnit: AMsspErrorUnit): AdsviserError {
