...
import com.avatye.sample.adcash.databinding.ActivityBannerWidget320x50Binding
import com.avatye.sample.adcash.extraParcel
import com.avatye.sample.adcash.launch

class BannerWidget320X50Activity : AppBaseActivity() {

    companion object {
        fun open(activity: Activity, parcel: PlacementParcel) {
            activity.launch(
                intent = Intent(activity, BannerWidget320X50Activity::class.java).apply {
    ...
    private val vb: ActivityBannerWidget320x50Binding by lazy {
        ActivityBannerWidget320x50Binding.inflate(LayoutInflater.from(this))
    }
