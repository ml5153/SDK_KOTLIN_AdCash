...
import com.avatye.sample.adcash.databinding.ActivityBannerDynamicBinding
import com.avatye.sample.adcash.extraParcel
import com.avatye.sample.adcash.launch

class BannerDynamicActivity : AppBaseActivity() {

    companion object {
        fun open(activity: Activity, parcel: PlacementParcel) {
            activity.launch(
                intent = Intent(activity, BannerDynamicActivity::class.java).apply {
    ...
    private val vb: ActivityBannerDynamicBinding by lazy {
        ActivityBannerDynamicBinding.inflate(LayoutInflater.from(this))
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        vb.buttonLoad.setOnClickListener {
            ...
            bannerAdLoader = BannerAdLoader(
                context = this@BannerDynamicActivity,
                placementId = pid,
                ...
                listener = object : BannerAdLoader.BannerListener {
                    override fun onLoaded(adView: View, size: BannerAdSize) {
                        ...
                        Handler(Looper.getMainLooper()).post {
                            Toast.makeText(this@BannerDynamicActivity, "Loader::onLoaded", Toast.LENGTH_SHORT).show()
                        }
                    }
                    ...
                    override fun onFailed(error: AdError) {
                        ...
                        Handler(Looper.getMainLooper()).post {
                            Toast.makeText(this@BannerDynamicActivity, "onFailed::${error.errorMessage}", Toast.LENGTH_SHORT).show()
                        }
                    }

                    override fun onClicked() {
                        Handler(Looper.getMainLooper()).post {
                            Toast.makeText(this@BannerDynamicActivity, "onClicked", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
            )
